...
void view_schedule(const void *S) {
    assert(S);
    const schedule_t *nS = (const schedule_t *)S;
...
void save_schedule(schedule_t *S, const char *filename) {
    ...
    for (int m = 0; m < get_num_machines(S); m++) {
        ...
        while (iterator) {
            schedule_node_t *node = (schedule_node_t *)get_list_node_data(iterator);
...
int find_empty_machine(schedule_t *S, unsigned long time) {
    ...
    for (int m = 0; m < get_num_machines(S); m++) {
        ...
        if (list_is_empty(machine_schedule) || time > get_schedule_node_end_time(
                (const schedule_node_t *)get_list_node_data((const list_node_t *)get_list_tail(machine_schedule)))) {
            return m;
        }
    }
    return -1;
...
int find_machine_to_interrupt(const schedule_t *S, const unsigned long time, const unsigned long processing_time) {
    ...
    for (int m = 0; m < get_num_machines(S); m++) {
        ...
        if (processing_time < get_schedule_node_end_time((const schedule_node_t *)get_list_node_data(get_list_tail(machine_schedule))) - time) {
            return m;
        }
    }
    return -1;
...
void add_task_to_schedule(const schedule_t *S, task_t *task, const int machine, const unsigned long bt, const unsigned long et) {
    ...
    if (list_is_empty(machine_schedule) || get_schedule_node_end_time((const schedule_node_t *)get_list_node_data(get_list_tail(machine_schedule))) <= bt) {
...
unsigned long preempt_task(schedule_t *S, int machine, unsigned long new_et) {
    assert(S);
    list_t *machine_schedule = get_schedule_of_machine(S, machine);
    schedule_node_t *last_node = (schedule_node_t *)get_list_node_data(get_list_tail(machine_schedule));
...
unsigned long get_completion_time(schedule_t *S, const int m) {
    ...
    return get_schedule_node_end_time((const schedule_node_t *)get_list_node_data(get_list_tail(machine_schedule)));
}
...
unsigned long get_lmax_for_machine(const schedule_t* S, const int machine) {
    ...
    while (iterator) {
        const schedule_node_t * node = (const schedule_node_t *)get_list_node_data(iterator);
